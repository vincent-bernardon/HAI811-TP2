...
public class Ex5 extends AppCompatActivity implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        float mangetude = (float) Math.sqrt(x*x + y*y + z*z);

        //si la magnitude est superieur au dépase le seuil positif ou négatif alors on alume le flash, ou on l'éteint si elle est alumé
        if(mangetude > seuil || mangetude < -seuil){
            onoff = !onoff;
            TextView textView = findViewById(R.id.offon);
            textView.setText(onoff ? "Flash : ON" : "Flash : OFF");
